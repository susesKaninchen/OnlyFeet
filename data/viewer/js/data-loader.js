...
const DataLoader = (() => {
  ...
  /**
   * Load a session from the device via REST API.
   * sessionFiles: array of path strings from /api/files
   * sessionName:  string, e.g. "data0"
   * onProgress:   optional callback(loaded, total)
   */
  async function loadFromDevice(sessionFiles, sessionName, onProgress) {
    const pktJsons   = {};
    const pktImgs    = {};
    const pktMidImgs = {};
    const pktWavs    = {};
    ...
    for (const path of sessionFiles) {
      ...
      let m;
      if      ((m = reJson.exec(name)))   pktJsons[parseInt(m[1])]   = path;
      else if ((m = reMidImg.exec(name))) pktMidImgs[parseInt(m[1])] = path;
      else if ((m = reImg.exec(name)))    pktImgs[parseInt(m[1])]    = path;
      else if ((m = reWav.exec(name)))    pktWavs[parseInt(m[1])]    = path;
    }

    const indices = new Set([
      ...Object.keys(pktJsons), ...Object.keys(pktImgs),
      ...Object.keys(pktMidImgs), ...Object.keys(pktWavs)
    ].map(Number));
    const sortedIndices = [...indices].sort((a, b) => a - b);
    ...
    for (const idx of sortedIndices) {
      ...
      if (pktJsons[idx]) {
        const r  = await fetch(`/api/download?path=${encodeURIComponent(pktJsons[idx])}`);
        pkt.json  = JSON.parse(await r.text());
        pkt.ts    = pkt.json.ts;
        pkt.midTs = pkt.json.midTs ?? null;
        pkt.endTs = pkt.json.endTs ?? null;
      }
      if (pktImgs[idx]) {
        const r = await fetch(`/api/download?path=${encodeURIComponent(pktImgs[idx])}`);
        pkt.imgUrl = URL.createObjectURL(await r.blob());
      }
      if (pktMidImgs[idx]) {
        const r = await fetch(`/api/download?path=${encodeURIComponent(pktMidImgs[idx])}`);
        pkt.midImgUrl = URL.createObjectURL(await r.blob());
      }
      if (pktWavs[idx]) {
        const r    = await fetch(`/api/download?path=${encodeURIComponent(pktWavs[idx])}`);
        const blob = await r.blob();
        pkt.wavUrl  = URL.createObjectURL(blob);
        pkt.wavFile = new File([blob], pktWavs[idx].split('/').pop(), { type: 'audio/wav' });
      }
    ...
    const sessionWavUrl = await buildSessionWav(packets);
    return { folderName: sessionName, packets, sessionWavUrl };
  }

  return { load, loadFromDevice };
})();
